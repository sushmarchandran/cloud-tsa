...
if [ -z "${ISTIO_ANALYTICS_TRACE_BACKEND}" ] || [ -z "${ISTIO_ANALYTICS_TRACE_SERVER_URL}" ]; then
   ...
   export ISTIO_ANALYTICS_TRACE_SERVER_URL="http://localhost:16686"
fi
...
if [ -z "${ISTIO_ANALYTICS_SKYDIVE_HOST}" ]; then
  export ISTIO_ANALYTICS_SKYDIVE_HOST="http://9.4.193.143:30703"
  echo Setting ISTIO_ANALYTICS_SKYDIVE_HOST to $ISTIO_ANALYTICS_SKYDIVE_HOST
fi
